...
namespace gps {
	...
	void Model3DRGBA::LoadModel(std::string fileName)
	{
        std::string basePath = fileName.substr(0, fileName.find_last_of('/')) + "/";
		ReadOBJ(fileName, basePath);
	}

    void Model3DRGBA::LoadModel(std::string fileName, std::string basePath)
	{
		ReadOBJ(fileName, basePath);
	}

	// Draw each mesh from the model
	void Model3DRGBA::Draw(gps::Shader shaderProgram)
	{
	...
	void Model3DRGBA::ReadOBJ(std::string fileName, std::string basePath){
	...
	gps::Texture Model3DRGBA::LoadTexture(std::string path, std::string type) {
	...
	GLuint Model3DRGBA::ReadTextureFromFile(const char* file_name) {
		int x, y, n;
